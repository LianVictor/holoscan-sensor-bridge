    ...
    : headers_(DEFAULT_COE_HEADERS)
{
    ...
    memcpy(headers_.mac_header.mac_src, iface.mac, sizeof(headers_.mac_header.mac_src));
    init_socket(if_name);
}

COETransmitter::COETransmitter(const std::string& if_name, const std::array<uint8_t, ETHER_ADDR_LEN>& mac_src)
    : headers_(DEFAULT_COE_HEADERS)
{
