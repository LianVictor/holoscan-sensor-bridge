...
// This condition is only generated by host software.
constexpr uint32_t RESPONSE_HOST_ERROR = 0xF0;
...
public:
    explicit TransactionError(const std::string& arg)
        : runtime_error(arg)
    {
    }
};

class TimeoutError : public TransactionError {
public:
    explicit TimeoutError(const std::string& arg)
        : TransactionError(arg)
    {
    }
};
...
public:
    ...
    typedef enum {
        ...
        GPIO0 = 18,
        GPIO1 = 19,
        SIF_0_FRAME_START = 20,
        SIF_1_FRAME_START = 21,
    } Event;

    /**
     * Support for e.g. I2C transactions synchronized with Event.
     */
    class Sequencer {
    public:
        typedef enum {
    ...
    /**
     * Sequencer for software-triggered (SW_EVENT) sequences.
     */
    std::shared_ptr<Sequencer> software_sequencer();

    /**
     * Sequencer for FrameStart and FrameEnd events.
     */
    std::shared_ptr<Sequencer> sif0_frame_start_sequencer();
    std::shared_ptr<Sequencer> sif1_frame_start_sequencer();

    /**
     * Sequencer for GPIO-related events.
     */
    std::shared_ptr<Sequencer> gpio0_sequencer();
    std::shared_ptr<Sequencer> gpio1_sequencer();
    ...
    public:
        ...
        // Accessors for configured UART parameters
        virtual uint32_t baud_rate() const = 0;
        virtual uint32_t data_bits() const = 0;
        virtual uint32_t parity() const = 0;
        virtual uint32_t stop_bits() const = 0;
        virtual uint32_t flow_control() const = 0;
        virtual uint32_t tx_almost_empty_threshold() const = 0;
        virtual uint32_t tx_almost_full_threshold() const = 0;
        virtual uint32_t rx_almost_empty_threshold() const = 0;
        virtual uint32_t rx_almost_full_threshold() const = 0;
        ...
        virtual void uart_configure(uint32_t baud_rate, uint32_t data_bits, uint32_t parity, uint32_t stop_bits,
            ...
            = 0;
        virtual void uart_set_baud_rate(uint32_t baud_rate) = 0;
        virtual void uart_set_data_bits(uint32_t data_bits) = 0;
        virtual void uart_set_parity(uint32_t parity) = 0;
        virtual void uart_set_stop_bits(uint32_t stop_bits) = 0;
        virtual void uart_set_flow_control(uint32_t flow_control) = 0;
        virtual void uart_set_tx_fifo_threshold(uint32_t tx_almost_empty_threshold, uint32_t tx_almost_full_threshold) = 0;
        virtual void uart_set_rx_fifo_threshold(uint32_t rx_almost_empty_threshold, uint32_t rx_almost_full_threshold) = 0;
        virtual void uart_disable() = 0;
        virtual void uart_enable() = 0;
        virtual void uart_tx_disable() = 0;
        virtual void uart_tx_enable() = 0;
        virtual void uart_rx_disable() = 0;
        virtual void uart_rx_enable() = 0;
        virtual void uart_dump_registers() = 0;
        virtual void uart_enable_internal_loopback() = 0;
        virtual void uart_disable_internal_loopback() = 0;
        virtual void uart_clear_sticky_bits(uint32_t bitmask) = 0;
    };

    /**
     * @brief Get a UART object
     *
     * @param port_number UART port identifier (platform-specific)
     * @param baud_rate UART baud rate
     * @param data_bits Number of data bits (5, 6, 7, or 8)
     * @param parity Parity mode (0=None, 1=Odd, 2=Even)
     * @param stop_bits Number of stop bits (1, 15 for 1.5, or 2)
     * @param flow_control Flow control mode (0=None, 1=RTS/CTS)
     * @return std::shared_ptr<Uart>
     */
    std::shared_ptr<Uart> get_uart(uint32_t port_number = 0, uint32_t baud_rate = 115200,
        uint32_t data_bits = 8, uint32_t parity = 0, uint32_t stop_bits = 1, uint32_t flow_control = 0);
    ...
    /**
     * Listener interface for asynchronous event messages.
     *
     * Implementations receive callbacks for specific events when they occur.
     * Each method is called when the corresponding event fires, with the
     * event state (true = active, false = inactive) and timestamp.
     */
    class AsyncEventListener {
    public:
        virtual ~AsyncEventListener();

        // Default implementations do nothing - subclasses override only what they need.
        virtual void on_gpio0_event(uint32_t state, uint64_t timestamp_s, uint32_t timestamp_ns) { }
        virtual void on_gpio1_event(uint32_t state, uint64_t timestamp_s, uint32_t timestamp_ns) { }
        virtual void on_sif0_frame_start_event(uint32_t state, uint64_t timestamp_s, uint32_t timestamp_ns) { }
        virtual void on_sif0_frame_end_event(uint32_t state, uint64_t timestamp_s, uint32_t timestamp_ns) { }
        virtual void on_sif1_frame_start_event(uint32_t state, uint64_t timestamp_s, uint32_t timestamp_ns) { }
        virtual void on_sif1_frame_end_event(uint32_t state, uint64_t timestamp_s, uint32_t timestamp_ns) { }
    };

    /**
     * Register an async event listener that will be called from the
     * async event thread whenever an event packet is received.
     */
    void on_async_event(std::shared_ptr<AsyncEventListener> listener);
    ...
    /** Allow the application to hook into various error conditions. */
    virtual bool write_timeout_error(const std::string& message);
    virtual bool bad_write_response(uint32_t response_code, const std::string& message);
    virtual std::tuple<bool, std::vector<uint32_t>> bad_read_response(uint32_t response_code, const std::string& message);
    virtual uint32_t read_timeout_error(const std::string& message);
    virtual void malformed_response(const std::string& message);
...
private:
    ...
    std::vector<std::shared_ptr<AsyncEventListener>> async_event_listeners_;
    std::mutex async_event_listeners_mutex_;
    ...
    std::tuple<bool, std::vector<uint32_t>> read_uint32_singly_(uint32_t address, uint32_t count, const std::shared_ptr<Timeout>& in_timeout, bool sequence_check = true);
