...
<Tab title="Microchip MPF200-ETH-SENSOR-BRIDGE-R2">

Here are instructions to set up the Microchip MPF200-ETH-SENSOR-BRIDGE-R2 device and
...
Microchip MPF200-ETH-SENSOR-BRIDGE-R2 has the following connectors:

1. **SFP+ ports** - Two 10Gbps Ethernet ports which connect to the host system.
1. **Jetson camera connector J4** - This connector is used to interface with the Jetson Adapter Card which supports up to 4x 22-pin MIPI CSI-2 modules.
1. **12V DC Barrel Jack J10** - the sensor bridge device can be powered by using a 12V power supply
   with a minimum of 12V/2A connected to this port. If using 12V Barrel Jack for powering the sensor bridge device ensure that J36 is closed on pins 1 and 2(`5P0V_REG`). Follow the instructions in the
   [setup page](setup.mdx) to configure your host system.
1. **USB-C Power/UART J8** - alternatively the sensor bridge device can be powered by using USB-C with the J36 jumper closed on pins 2 and 3(`USB_C_5V`)

Microchip MPF200-ETH-SENSOR-BRIDGE-R2 reference applications are currently using the
...
1. For more details see the
   [Microchip MPF200-ETH-SENSOR-BRIDGE-R2](https://www.microchip.com/en-us/products/fpgas-and-plds/boards-and-kits/ethernet-sensor-bridge)
