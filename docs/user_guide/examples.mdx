...
## IMX477 player example

To run the video player IMX477 example on a ConnectX-enabled host, in the demo
container, run

```sh
  $ python3 examples/imx477_player.py
```

for unaccelerated hosts,

```sh
$ python3 examples/linux_imx477_player.py
```
...
for unaccelerated configurations with IMX477 camera,

```sh
$ python3 examples/linux_tao_peoplenet_imx477.py
```
...
for ConnectX-accelerated hosts, within the sensor bridge demo container, launch the Body
Pose estimation with IMX477 camera:

```sh
$ python3 examples/body_pose_estimation_imx477.py
```

for unaccelerated hosts with IMX477 camera, launch the Body Pose estimation example within the demo
container this way:

```sh
$ python3 examples/linux_body_pose_estimation_imx477.py
```
...
## Running the Stereo IMX274, Stereo IMX477, Leopard imaging Li VB1940 Eagle, and TauroTech DA326 Hawk examples
...
ConnectX-accelerated hosts with IMX477:

```sh
$ python3 examples/single_network_stereo_imx477_player.py
```

Unaccelerated hosts with IMX477:

```sh
$ python3 examples/linux_single_network_stereo_imx477_player.py
```
